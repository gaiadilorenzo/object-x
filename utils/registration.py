...
def compute_rotation_mse_and_mae(gt_rotation: np.ndarray, est_rotation: np.ndarray):
    ...
    gt_euler_angles = Rotation.from_matrix(gt_rotation).as_euler(
        "xyz", degrees=True
    )  # (3,)
    est_euler_angles = Rotation.from_matrix(est_rotation).as_euler(
        "xyz", degrees=True
    )  # (3,)
...
def compute_registration_error(
    gt_transform: np.ndarray, est_transform: np.ndarray, inverse_trans=False
):
