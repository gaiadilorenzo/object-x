...
def compute_sgar(sim, rank_list, e1i_idxs, e2i_idxs, modes):
    ...
    for mode in modes:
        if mode == "2":
            sim_argsorted_mode = sim_argsorted[:2]
        elif mode == "50":
            sim_argsorted_mode = sim_argsorted[: len(sim_argsorted) // 2]
        else:
            sim_argsorted_mode = sim_argsorted
...
def compute_node_corrs(rank_list, src_objects_count, k=1):
    ...
    for idx in range(src_objects_count):
        ...
        for k_idx in range(0, k):
            if entity_rank_list_k[k_idx] < src_objects_count:
                continue
...
def get_node_corrs_objects_ids(node_corrs, objects_ids, batch_offset):
    ...
    for node_corr in node_corrs:
        node_corrs_objects_ids.append(
            (
                objects_ids[node_corr[0] + batch_offset],
                objects_ids[node_corr[1] + batch_offset],
            )
        )
