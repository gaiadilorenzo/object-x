...
from models.backbones.sg_encoder import MultiModalEncoder
from src.models.model_utils import PatchCNN, TransformerEncoder
...
class PatchSGIEAligner(nn.Module):
    def __init__(
        ...
        if num_reduce > 0:
            ...
            from src.models.vlsg.src.models.GCVit.models import gc_vit

            reduce_list = [
                gc_vit.ReduceSize(dim=backbone_dim, keep_dim=True)
                for i in range(num_reduce)
            ]
            self.reduce_layers = nn.Sequential(*reduce_list)
        else:
            self.reduce_layers = EncodeChannelSize(
                backbone_dim, backbone_dim, norm_layer=nn.LayerNorm
            )

        # patch feature encoder
        self.img_transpose = img_transpose
        self.patch_encoder = Mlps(
            backbone_dim,
            hidden_features=patch_hidden_dims,
            out_features=patch_encoder_dim,
            drop=drop,
        )
        ...
        self.patch_gcn = (
            PatchCNN(d_model=patch_encoder_dim, num_layers=num_patch_gcn_layers)
            if self.num_patch_gcn_layers > 0
            else nn.Identity()
        )
        self.projection_layer = nn.Sequential(
            nn.Conv1d(
                400, obj_embedding_dim, kernel_size=1, groups=4
        ...
        if encoder is None:
            self.encoder = MultiModalEncoder(
                ...
                dropout=drop,
                img_aggregation_mode=multi_view_aggregator,
                use_pos_enc=obj_img_pos_enc,
                in_latent_dim=in_feature_dim,
                out_latent_dim=out_feature_dim,
                voxel_encoder=decoder_net,
                channels=voxel_channels,
            )
        else:
            self.encoder = encoder
        # self.obj_embedding_encoder = Mlps(
        #     obj_embedding_dim,
        #     hidden_features=obj_embedding_hidden_dims,
        #     out_features=obj_encoder_dim,
        #     drop=drop,
        # )
        ...
        if self.use_global_descriptor:
            ...
            self.patch_global_encoder = TransformerEncoder(
                num_layers=1,
                d_model_in=patch_encoder_dim,
                d_model_inner=256,
                d_model_out=global_descriptor_dim,
            )
            self.obj_global_encoder = TransformerEncoder(
                num_layers=2,
                d_model_in=obj_encoder_dim,
                d_model_inner=256,
                d_model_out=global_descriptor_dim,
            )
    ...
    def calculate_similarity(
        self,
        obj_3D_embeddings_norm,
        patch_features_norm,
        assoc_data_dict,
        obj_3D_embeddings_sim,
    ):
        scan_objs_idx = assoc_data_dict["scans_sg_obj_idxs"]
        scans_obj_embeddings_norm = obj_3D_embeddings_norm[scan_objs_idx, :]  # (N, C*)
        patch_obj_sim = torch.mm(
            patch_features_norm, scans_obj_embeddings_norm.permute(1, 0)
        )  # (P_H*P_W, N)
        patch_patch_sim = torch.mm(
            patch_features_norm, patch_features_norm.permute(1, 0)
        )  # (P_H*P_W, P_H*P_W)
        obj_obj_sim = obj_3D_embeddings_sim[scan_objs_idx, :][
            :, scan_objs_idx
        ]  # (N, N)
    ...
    def forward(self, data_dict):
        ...
        features = self.backbone(images)[
            -1
        ]  # (B, C', H/32, W/32); input channel first,output channel first
        features = _to_channel_last(features)
        patch_features = self.reduce_layers(
            features
        )  # (B, H/64, W/64, C'); input channel last,output channel last
        patch_features = self.patch_encoder(
            patch_features
        )  # (B, P_H, P_W, C*); input channel last,output channel last
        # to channel first
        ...
        obj_3D_embeddings_sim = torch.mm(
            obj_3D_embeddings_norm, obj_3D_embeddings_norm.permute(1, 0)
        )  # (O, O)
        ...
        embs[
            "patch_patch_sim_temp"
        ] = patch_patch_sim_temp_list  # B - [P_H*P_W, P_H*P_W]
    ...
    def forward_with_patch_features(self, data_dict):
        ...
        patch_features = self.projection_layer(patch_features)
        patch_features = patch_features.reshape(
            patch_features.shape[0], patch_features.shape[1], 16, 9
        )
        patch_features = _to_channel_last(patch_features)
        patch_features = patch_features.flatten(1, 2)  # (B, P_H*P_W, C*)
        ...
        if self.use_global_descriptor:
            (
                patch_global_descriptor,
                obj_global_descriptors,
            ) = self.forward_global_descriptor(
    ...
    def forward_scene_graph(self, data_dict):
        object_embeddings = self.encoder(data_dict)
    ...
    def forward_global_descriptor(self, patch_features, obj_features, data_dict):
        ...
        patch_global_descriptor = self.patch_global_encoder.forward_cls(
            patch_features
        )  # (B C*)
        ...
        for scan_id, obj_idxs in sg_obj_idxs.items():
            ...
            obj_global_descriptor = self.obj_global_encoder.forward_cls(
                obj_features_scan
            )
