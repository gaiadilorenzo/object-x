...
class TransformerEncoderLayer(nn.Module):
    def __init__(
        self, d_model: int = 256, nhead: int = 4, acti_layer=nn.GELU()
    ) -> None:
        super().__init__()
...
class TransformerEncoder(nn.Module):
    def __init__(
        ...
        self.cls_token = nn.Parameter(
            F.normalize(torch.ones(1, 1, d_model_inner), dim=-1), requires_grad=True
        )

        self.map_in = nn.Linear(d_model_in, d_model_inner)
        self.layers = nn.ModuleList(
            [
                TransformerEncoderLayer(d_model_inner, num_heads, acti_layer)
