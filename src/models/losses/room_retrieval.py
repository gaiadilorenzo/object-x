...
class ICLLossBothSidesSumOutLog(nn.Module):
    ...
    def calculate_loss(
        ...
        loss_patch_side = delta_E1i_E2i / (
            delta_E1i_E2i + sum_delta_E1i_E1j + sum_delta_E1i_E2j + epsilon
        )
        ...
        delta_F1i_F1j_all_objs = (
            (f1j * obj_obj_sim_exp).sum(dim=-1).reshape(-1, 1)
        )  # (O,1)
        sum_delta_F1i_F1j = delta_F1i_F1j_all_objs.gather(
            0, gt_matched_obj_idxs
        ).reshape(
            -1
        )  # (N_P,1)
        obj_patch_sim_exp = patch_obj_sim_exp.transpose(0, 1)  # (O, N_P)
        delta_F1i_F2j_all_objs = (
            (f2j * obj_patch_sim_exp).sum(dim=-1).reshape(-1, 1)
        )  # (O,1)
        sum_delta_F1i_F2j = delta_F1i_F2j_all_objs.gather(
            0, gt_matched_obj_idxs
        ).reshape(
            -1
        )  # (N_P,1)
        loss_obj_side = delta_F1i_F2i / (
            delta_F1i_F2i + sum_delta_F1i_F1j + sum_delta_F1i_F2j + epsilon
        )  # (N_P)

        loss, matched_obj_labels = None, None
        if e1i_valid.any():
            loss = -torch.log(
                loss_patch_side[e1i_valid] + epsilon
            ) * alpha + -torch.log(loss_obj_side[e1i_valid] + epsilon) * (1 - alpha)
            matched_obj_idxs = torch.argmax(patch_obj_sim, dim=1).reshape(
                -1, 1
            )  # (N_P)
    ...
    def forward_global_match_item(self, embs, data_dict, key=""):
        ...
        for batch_i in range(data_dict["batch_size"]):
            ...
            for candidate_scan_id in candidate_scan_ids:
                scan_global_descri = obj_global_descriptors[candidate_scan_id].squeeze(
                    0
                )  # (D)
                cos_sim = F.cosine_similarity(
                    patch_global_desc, scan_global_descri, dim=0
                )
            ...
            loc_sucess_batch = (
                is_success
                if loc_sucess_batch is None
                else torch.cat([loc_sucess_batch, is_success])
            )
    ...
    def forward(self, embs, data_dict):
        ...
        for batch_i in range(batch_size):
            ...
            loss_batch, matched_success_batch = self.forward_item(
                embs, assoc_data_dict, batch_i
            )
            if loss_batch is not None:
                loss_batch_NT = (
                    loss_batch
                    if loss_batch_NT is None
                    else torch.cat([loss_batch_NT, loss_batch])
                )
                matched_success_batch_NT = (
                    matched_success_batch
                    if matched_success_batch_NT is None
            ...
            if self.use_temporal:
                ...
                if loss_batch is not None:
                    loss_batch_T = (
                        loss_batch
                        if loss_batch_T is None
                        else torch.cat([loss_batch_T, loss_batch])
                    )
                    matched_success_batch_T = (
                        matched_success_batch
                        if matched_success_batch_T is None
        ...
        if not self.use_temporal:
            loss_dict["loss"] = (
                loss_batch_NT.mean()
                if loss_batch_NT is not None
                else torch.tensor([0.0]).to(loss_batch_NT.device)
            )
            loss_dict["matched_success_ratio"] = (
                matched_success_batch_NT.mean()
                if matched_success_batch_NT is not None
                else torch.tensor([0.0]).to(loss_batch_NT.device)
            )
        else:
            loss_dict["loss"] = (
                (loss_batch_NT.mean() + loss_batch_T.mean()) / 2
                if loss_batch_NT is not None and loss_batch_T is not None
            ...
            loss_dict["loss_T"] = (
                loss_batch_T.mean()
                if loss_batch_T is not None
                else torch.tensor([0.0]).cuda()
            )
            loss_dict["matched_success_ratio_NT"] = (
                matched_success_batch_NT.mean()
                if matched_success_batch_NT is not None
                else torch.tensor([0.0]).cuda()
            )
            loss_dict["matched_success_ratio_T"] = (
                matched_success_batch_T.mean()
                if matched_success_batch_T is not None
                else torch.tensor([0.0]).cuda()
            )
        ...
        with torch.no_grad():
            if self.use_global_descriptor:
                (
                    loss_batch_global_NT,
                    sucess_global_batch_NT,
                ) = self.forward_global_match_item(embs, data_dict)
                if self.use_temporal:
                    (
                        loss_batch_global_T,
                        sucess_global_batch_T,
                    ) = self.forward_global_match_item(embs, data_dict, key="_temp")
                    ...
                    loss_dict["loss_global"] = (
                        loss_batch_global_NT.mean() + loss_batch_global_T.mean()
                    ) / 2
                    loss_dict["success_ratio_global"] = (
                        sucess_global_batch_NT.mean() + sucess_global_batch_T.mean()
                    ) / 2
                else:
                    loss_dict["loss_global_NT"] = loss_batch_global_NT.mean()
                    loss_dict["success_ratio_global_NT"] = sucess_global_batch_NT.mean()
    ...
    def roomRetrieval(self, embs, data_dict, key=""):
        ...
        for batch_i in range(batch_size):
            ...
            for candidate_scan_id in candata_scan_obj_idxs.keys():
                ...
                matched_candidate_obj_idxs = torch.argmax(
                    patch_candidate_obj_sim, dim=1
                ).reshape(
                    -1, 1
                )  # (N_P)
                matched_candidate_obj_sim = patch_candidate_obj_sim.gather(
                    1, matched_candidate_obj_idxs
                )
                # get scores
                candidate_room_scores_3[candidate_scan_id] = torch.sum(
                    matched_candidate_obj_sim
                )

            # get scan id sorted by given scores
            score_to_used = candidate_room_scores_3
            sorted_scan_id_score = [
                k
                for k, v in sorted(
                    score_to_used.items(), key=lambda item: item[1], reverse=True
                )
            ]

            total += 1
            if (
                target_scan_id in sorted_scan_id_score[:1]
                and score_to_used[target_scan_id] > 0
            ):
                retrieval_result["R@1{}".format(key)] += 1
            if (
                target_scan_id in sorted_scan_id_score[:3]
                and score_to_used[target_scan_id] > 0
            ):
                retrieval_result["R@3{}".format(key)] += 1
            if (
                target_scan_id in sorted_scan_id_score[:5]
                and score_to_used[target_scan_id] > 0
            ):
                retrieval_result["R@5{}".format(key)] += 1

        retrieval_result["R@1{}".format(key)] = (
            retrieval_result["R@1{}".format(key)] * 1.0 / total
        )
        retrieval_result["R@3{}".format(key)] = (
            retrieval_result["R@3{}".format(key)] * 1.0 / total
        )
        retrieval_result["R@5{}".format(key)] = (
            retrieval_result["R@5{}".format(key)] * 1.0 / total
        )
